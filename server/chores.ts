...
choresRouter.post("/", async (req, res) => {
  try {
    ...
    if (!apartmentId) {
      return res.status(400).json({ success: false, error: "Apartment not found" });
    }

    const query = `
      INSERT INTO choresTable (aptid, userid, description, isrepeat, repeat, isdone)
      VALUES ($1, $2, $3, $4, $5, false)
      RETURNING *`;

    const values = [apartmentId, userId, description, isRepeat, repeat];
...
async function getAppartmentId(userId: number): Promise<number> {
 ...
 // Add this route to your choresRouter
choresRouter.delete("/:id", async (req, res) => {
  try {
    ...
    res.status(200).json({ success: true, message: "Chore deleted successfully", chore: result[0] });
  } catch (error: any) {
