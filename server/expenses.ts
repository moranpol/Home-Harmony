...
expensesRouter.get("/balance/:userId", async (req, res) => {
  ...
  try {
    const userId = Number(req.params.userId);
    const apartmentId = await getAppartmentId(userId);
    const sumAllCost = await queryRunner.query(
      `SELECT SUM(e.cost) FROM expenses e WHERE e.apt_id='${apartmentId}'`
    );
    const numRommates = await queryRunner.query(
      `SELECT COUNT(*) FROM userstable u WHERE u.aptid='${apartmentId}'`
    );
    const sumUserCost = await queryRunner.query(
      `SELECT SUM(e.cost) FROM expenses e WHERE e.apt_id='${apartmentId}' AND e.user_id='${userId}'`
    );
    const result =
      sumUserCost[0]?.sum - sumAllCost[0]?.sum / numRommates[0]?.count;
...
expensesRouter.post("/add", async (req, res) => {
  try {
    console.log("try add expenseId: ", req.body.userId);
